...
Categories <- end_year_population_totals_long.asy %>%
  ...
  mutate( valabel = scales::label_number_si(accuracy = 0.1)(Value2))
...
Categories$Population.type.label.short <- factor(Categories$Population.type.label.short,
                                                 levels = c("Refugees","Asylum-seekers",
                                                            "Venezuelans Abroad",
                                                            "IDPs",
                                                            "Stateless Persons",
                                                            "Others of Concern")) 
...
ggplot(Categories, aes(x = Year, 
  ...
  labs(title = "How Categories of Population of concern to UNHCR are evolving over time?",
       subtitle = paste0("Data as of ",lastyear, "  in ",thiscountry ), 
       x = "",
       y = "",
...
if( nrow(Origin) == 0 ){
  cat(paste0("There's no recorded countries of Origin i ",thiscountry )) 
  } else {
...
ggplot(Origin, aes(x = reorder(CountryOriginName, Value2), ## Reordering country by Value
  ...
  labs(title = paste0("What are the main Origin of Forced Displacement across Borders?" ),
       subtitle = paste0("Top 10 Origin -Data as of ",lastyear, "  in ",thiscountry ), 
       x = " ",
...
if( nrow(Destination) ==  0 ){
  cat(paste0("There's no recorded countries of destination i ",thiscountry ))
  
} else {


#Make plot
ggplot(Destination, aes(x = reorder(CountryAsylumName, Value2), ## Reordering country by Value
                           y = Value2)) +
  geom_bar(stat = "identity", 
           position = "identity", 
           fill = "#0072bc") + # here we configure that it will be bar chart+
  ...
  coord_flip() + # Add `coord_flip()` to make your vertical bars horizontal:
  ## and the chart labels
  labs(title = paste0("What are the main destination of Forcibly Displaced People?" ),
       
       subtitle = paste0("Top Destination Countries -Data as of ",lastyear, "  in ",thiscountry ), 
       x = "",
       y = "",
       caption = "Data: UNHCR Refugee Population Statistics Database; Visualisation: UnhcrDataPackage.\n Forced Displacement includes Refugees, Asylym Seekers and Venezuelan Displaced Abroad Population Group.") +
...
if( nrow(demographics1) ==  0 ){
  cat(paste0("There's no recorded Gender disaggregation for Forcibly Displaced People across Borders in ",thiscountry ))
  ...
  if (totprop == 0 ) {
    cat(paste0("There's no recorded Gender disaggregation for all of the ",tot, " persons in", thiscountry ))
...
# Make the Network
sankey <- networkD3::sankeyNetwork(Links = linksallDecision, 
                              Nodes = nodesDecision,
                              Source = "IDsource", 
                              Target = "IDtarget",
                              Value = "value", 
...
sankey <- htmlwidgets::prependContent(sankey, htmltools::tags$h1(paste0("Aylum Processing in ", thiscountry)))
sankey <- htmlwidgets::appendContent(sankey, htmltools::tags$p("Data: UNHCR Refugee Population Statistics Database; Visualisation: UnhcrDataPackage"))

sankey
# you save it as an html
networkD3::saveNetwork(sankey, paste0(getwd(),"/decision/decision", thiscountry,".html"))
...
if( nrow(Solution) ==  0 ){
  cat(paste0("There's no recorded solutions in ",thiscountry ))
...
ggplot(Solution, aes(x = Year, y = Value2  )) + 
  ...
  labs(title = "What are the trends in terms of Solutions?",
       subtitle = paste0("Data for Forcibly Displaced People across Borders as of ",lastyear, " filtered for ", thiscountry), 
       x = "",
       y = "",
       caption = "Data: UNHCR Refugee Population Statistics Database; Visualisation: UnhcrDataPackage.\n Forced Displacement includes Refugees, Asylym Seekers and Venezuelan Displaced Abroad Population Groups.")
}
...
if( nrow(displaced) ==  0 ){
  cat(paste0("There's no recorded Forcibly Displaced Poeple across Borders in ",thiscountry ))
