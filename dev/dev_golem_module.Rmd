...
The app includes 2 shared module - 

  * One for the selection of countries and year - that will be then passed on to all the plots
  * One that takes the global reactive values - and the type of plot (form the library) as a parameter 
...
#' @import shiny
#' @keywords internal
mod_1migrants_server <- function(id, reactiveParameters){
  moduleServer( id, function(input, output, session){
    ns <- session$ns 
   ...
   if (input$subtitle != "") { p <- p + labs(subtitle = input$subtitle)}
    ...
    output$dl <- downloadHandler(
            filename = function() {
              paste('plot_ctr_disp_migrant-', Sys.Date(), '.png', sep='')
            },
            content = function(con) {
              ggsave(con,
                     reactiveParameters$p, 
...
```{r examples-mod_1migrants}

```

```{r tests-mod_1migrants}
# test_that("mod_1migrants works", {
# 
# })
```
...
mod_migrants_ui <- function(id){
  ...
  tagList(
   tabsetPanel(type = "tabs",
         tabPanel(title= "Plot Migrant",
                    mod_plotviz_ui("migrants1", thisPlot = "plot_ctr_disp_migrant" ) ),
         tabPanel(title= "Plot Migrant 2",
                    mod_plotviz_ui("migrants2", thisPlot = "plot_ctr_disp_migrant" ) ),
         tabPanel(title= "Plot Migrant 3",
                    mod_plotviz_ui("migrants3", thisPlot = "plot_ctr_disp_migrant" ) )
      )
  )
...
mod_migrants_server <- function(id, reactiveParameters){
  moduleServer( id, function(input, output, session){
    ns <- session$ns 
       ...
       mod_plotviz_server("migrants2", thisPlot = "plot_ctr_disp_migrant", reactiveParameters )
       mod_plotviz_server("migrants3", thisPlot = "plot_ctr_disp_migrant", reactiveParameters )
